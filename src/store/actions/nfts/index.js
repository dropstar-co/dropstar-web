...
export const fetchNfts = nftId => {
  return async dispatch => {
    try {
      ...
      const response = await axios(axiosPayload(`${BASE_URL}nft/${nftId}`, '', 'get'));
...
export const fetchNftsBids = nftId => {
  return async dispatch => {
    try {
      dispatch(setAppLoading(true));
      const response = await axios(axiosPayload(`${BASE_URL}bid/nft/${nftId}`, '', 'get'));
      dispatch(setNftStatus('fetching'));
      if (response && response.status === 200) {
        dispatch(setAppLoading(false));
        dispatch(setNftStatus('completed'));
...
export const postBid = data => {
  return async dispatch => {
    try {
      const response = await axios(axiosPayload(`${BASE_URL}bid/complete`, data, 'post'));
